...
export const RequestExplorer = () => {
  ...
  const load = async () => {
    try {
      const data = await apiFetch<RequestRow[]>('/requests/?limit=50');
      setRequests(data);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="glass-panel p-6 w-full">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="text-center py-8 text-slate-500 animate-pulse">Loading requests...</td></tr>
            ) : visible.length === 0 ? (
              <tr><td colSpan={7} className="text-center py-8 text-slate-500">No requests found.</td></tr>
            ) : (
              visible.map((req) => (
